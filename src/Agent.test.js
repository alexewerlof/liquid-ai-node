...
describe('Agent', () => {
    ...
    test('should invoke tools and loop up to MAX_CALLS', async () => {
        ...
        toolbox.addTool('get_temp', 'get temperature').fn(() => ({ temp: 22 })).prm('loc: string*', 'location');

        llm.responses.push([{
            generated_text: [{
                role: 'assistant',
                content: '',
                tool_calls: [{ id: 'call_1', type: 'function', function: { name: 'get_temp', arguments: '{"loc":"Paris"}' } }]
            }]
        }]);
        llm.responses.push([{
